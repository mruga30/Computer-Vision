...
def splitt(img):
    """
    Splits a image (a height x width x 4 ndarray) into 4 ndarrays, 1 for each channel.

    Args:
        img:    A height x width x 3 channel ndarray.

    Returns:
        A 4-tuple of the r, g, and b and transparency channels.
    """
    if img.shape[2] != 3:
        raise ValueError('The split function requires a 3-channel input image')
    else:
        #image is split into four channels-r,g,b,t by splitting the 4D image array into 2D arrays
    	t = img[:,:,3]
    	b = img[:,:,2]
    	g = img[:,:,1]
    	r = img[:,:,0]
...
def merget(r, g, b,t):
    ...
    # """
    #the r,g,b channels are merged by converting 3 2d arrays into one 3d array 
    result = np.empty([h,w,4], dtype = 'uint8')
    result = np.dstack((r,g,b,t))
    return result
...
if __name__ == '__main__':
    ...
    if ch == 3:
    	...
    	logging.info('Convolving the grayscale image')
    	resultImage = convolve(inputImage, kernel)

    else:
    	logging.info('Splitting it into 4 channels')
    	(r, g, b,t) = splitt(inputImage)
